`pirn_data.frames` provides Tier 2 in-memory frame knots for Polars, pandas, PyArrow, and DuckDB — it does not push work to external engines; for out-of-core or distributed execution see `pirn_data.lazy` (Tier 3).
...
- **Frame knots are in-memory.** For datasets that do not fit in RAM, use Tier 3 lazy knots (`pirn_data.lazy`) with Dask, Ibis, or Spark.
