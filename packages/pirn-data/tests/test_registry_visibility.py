"""Registry-visibility acceptance test for pirn-data (PIR-856).

``pirn_data/__init__.py`` calls ``Registry.fill_registry`` at import time and
...
def _walk_knot_subclasses(package: object) -> tuple[set[str], list[str]]:
    ...
    module-level alias (``OtherName = SomeClass``) binds a second name to the
    *same* class object; sweet_tea's
...
def test_registry_holds_every_knot_subclass_when_extras_present() -> None:
    found, failed = _walk_knot_subclasses(pirn_data)
    if failed:
        pytest.skip(
            "an optional dependency is missing for: "
            + ", ".join(sorted(failed))
            + " — install pirn-data[all-frames] / [all-lazy] to run this test"
        )
