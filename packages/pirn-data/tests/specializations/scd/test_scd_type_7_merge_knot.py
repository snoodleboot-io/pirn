...
def _make_knot(pool: MagicMock | None = None, **kwargs: Any) -> ScdType7MergeKnot:
    if pool is None:
        pool = _make_pool()
    defaults: dict[str, Any] = {
        ...
        "surrogate_key_column": _SURROGATE_COL,
    ...
    with Tapestry():
        return ScdType7MergeKnot(**defaults, _config=KnotConfig(id="scd7"))


class TestScdType7MergeKnotConstruction(unittest.TestCase):
    def test_valid_construction(self) -> None:
        self.assertIsInstance(_make_knot(), ScdType7MergeKnot)

    def test_static_query_methods(self) -> None:
        self.assertIn(
            "dim_customer",
            ScdType7Queries.select_query("dim_customer", ("id", "region"), "is_current"),
        )
        self.assertIn(
            "INSERT INTO dim_customer",
            ScdType7Queries.insert_query(
                "dim_customer", "scd_id", ("id", "region"), "valid_from", "valid_to", "is_current"
            ),
        )
        self.assertIn(
            "dim_customer",
            ScdType7Queries.expire_query("dim_customer", ("id",), "valid_to", "is_current"),
        )


class TestScdType7MergeKnotProcess(unittest.IsolatedAsyncioTestCase):
    async def _call(
        self, k: ScdType7MergeKnot, rows: Any, pool: MagicMock, **overrides: Any
    ) -> dict[str, int]:
        args: dict[str, Any] = {
            "rows": rows,
            "target_pool": pool,
    ...
    async def test_inserts_new_rows_with_surrogate(self) -> None:
        ...
        k = _make_knot(pool)
        result = await self._call(k, [(1, "EU")], pool)
        self.assertEqual(result["inserted"], 1)
        self.assertEqual(result["expired"], 0)

    async def test_expires_and_inserts_changed_row(self) -> None:
        pool = _make_pool()
        pool.fetch_all = AsyncMock(side_effect=[[(1, "EU")], [(5,)]])
        k = _make_knot(pool)
...
class TestValidation(unittest.IsolatedAsyncioTestCase):
    async def _call(self, k: ScdType7MergeKnot, **overrides: Any) -> Any:
        pool = _make_pool()
        args: dict[str, Any] = {
            "rows": [],
            "target_pool": pool,
    ...
    async def test_rejects_bookkeeping_columns_in_column_names(self) -> None:
        k = _make_knot()
        with self.assertRaises(ValueError):
            await self._call(
                k,
