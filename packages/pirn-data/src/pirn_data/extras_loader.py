"""Per-domain optional-extras loader for ``pirn_data``.

The ``pirn_data`` package instantiates :class:`ExtrasLoader` with its
...
class ExtrasLoader:
    ...
    def require(self) -> None:
        ...
        pretty = ", ".join(missing)
        raise ImportError(
            f"pirn_data[{self._extra_name}] requires {pretty}. "
            f"Install with: pip install 'pirn-data[{self._extra_name}]'"
        )
