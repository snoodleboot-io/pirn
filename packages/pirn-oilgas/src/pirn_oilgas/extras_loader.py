"""Per-domain optional-extras loader for ``pirn_oilgas``.

The ``pirn_oilgas`` package instantiates :class:`ExtrasLoader` with its
...
class ExtrasLoader:
    ...
    def require(self) -> None:
        ...
        pretty = ", ".join(missing)
        raise ImportError(
            f"pirn_oilgas[{self._extra_name}] requires {pretty}. "
            f"Install with: pip install 'pirn-oilgas[{self._extra_name}]'"
        )
