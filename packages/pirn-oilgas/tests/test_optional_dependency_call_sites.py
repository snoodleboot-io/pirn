"""Every optional import in pirn_oilgas names this package and one of its real extras.

pirn_oilgas imports its optional backends through
:meth:`pirn.core.optional_dependency.OptionalDependency.require`. The install hint
that call raises is only useful if it names ``pirn-oilgas`` and an extra that
...
class TestOptionalDependencyCallSites:
    ...
    def _require_calls() -> list[tuple[str, ast.Call]]:
        ...
        for package_dir in pirn_oilgas.__path__:
    ...
    def test_every_call_names_this_package_and_a_declared_extra(self) -> None:
        ...
        for location, call in self._require_calls():
            ...
            if not (isinstance(package, ast.Constant) and package.value == "pirn-oilgas"):
                wrong.append(f"{location}: package is not 'pirn-oilgas'")
    ...
    def test_a_missing_module_names_the_package_extra(self) -> None:
        ...
        # Act / Assert
        with pytest.raises(ImportError, match=rf'pip install "pirn-oilgas\[{extra}\]"'):
            OptionalDependency.require(
                "pirn_oilgas_absent_backend", extra=extra, package="pirn-oilgas"
            )
