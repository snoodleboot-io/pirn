"""``SeismicQCCheck`` — quality-check seismic data and raise ValueError on failing datasets.
...
class SeismicQCCheck(Knot):
    """Check seismic data quality; raise on fold or null-percentage violations."""
    ...
    async def process(
        ...
        **_: Any,
        ...
        if not isinstance(max_null_pct, (int, float)):
            raise TypeError("SeismicQCCheck: max_null_pct must be numeric")
        if not (0 <= max_null_pct <= 100):
            raise ValueError("SeismicQCCheck: max_null_pct must be in [0, 100]")
        if not isinstance(min_fold, int):
            raise TypeError("SeismicQCCheck: min_fold must be an int")
        if min_fold <= 0:
            raise ValueError("SeismicQCCheck: min_fold must be positive")
        if not isinstance(max_amplitude, (int, float)):
            raise TypeError("SeismicQCCheck: max_amplitude must be numeric")
        if max_amplitude <= 0:
            raise ValueError("SeismicQCCheck: max_amplitude must be positive")
        if not isinstance(data, dict):
            raise TypeError("SeismicQCCheck: data must be a dict")
        fold: int = int(data.get("fold", 0))
        traces: list[Any] = data.get("traces", [])
        if fold < min_fold:
            raise ValueError(f"SeismicQCCheck: fold {fold} is below minimum {min_fold}")
        null_count = sum(1 for t in traces if t is None)
        null_pct = (null_count / len(traces) * 100.0) if traces else 0.0
        if null_pct > max_null_pct:
            raise ValueError(f"SeismicQCCheck: null_pct {null_pct:.1f} exceeds max {max_null_pct}")
        return {
            "passed": True,
