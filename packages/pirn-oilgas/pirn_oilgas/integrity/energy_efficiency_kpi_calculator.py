...
class EnergyEfficiencyKpiCalculator(Knot):
    ...
    def _compute(energy_consumption: ScadaPayload, production: ScadaPayload) -> dict[str, float]:
        aligned_count = min(len(energy_consumption.data), len(production.data))
        ...
        e_interval_hr = energy_consumption.metadata.sample_interval_sec / 3600.0
        p_interval_day = production.metadata.sample_interval_sec / 86400.0

        total_kwh = float(np.sum(energy_consumption.data[:aligned_count]) * e_interval_hr)
