"""``CosineSimilarity`` — backend-free cosine similarity of two vectors."""
...
class CosineSimilarity:
    ...
    def compute(self, a: Sequence[float], b: Sequence[float]) -> float:
        ...
        maximally dissimilar) rather than raising, so an empty-string embedding
        does not crash a metric.
        ...
        Returns:
            The cosine similarity in ``[-1.0, 1.0]`` (``0.0`` if either vector
            has zero magnitude).
        ...
        if len(a) != len(b):
            raise ValueError(
                f"CosineSimilarity: vectors must be equal length, got {len(a)} and {len(b)}"
            )
        dot = 0.0
        norm_a = 0.0
        norm_b = 0.0
