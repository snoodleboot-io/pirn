...
class JudgeCalibration:
    """Compare an :class:`EvaluationJudge`'s rubric scores against human gold labels.
    ...
    def __init__(self, *, judge: EvaluationJudge, tolerance: float = 0.1) -> None:
        """Store the judge under test and the agreement tolerance.

        Args:
            judge: The configured :class:`EvaluationJudge` to calibrate.
            tolerance: Maximum absolute score difference still counted as
                agreement.

        Raises:
            TypeError: If ``judge`` is not an :class:`EvaluationJudge` or ``tolerance``
                is not a real number.
            ValueError: If ``tolerance`` is negative.
        """
        if not isinstance(judge, EvaluationJudge):
            raise TypeError(
                f"JudgeCalibration: judge must be an EvaluationJudge, got {type(judge).__name__}"
            )
