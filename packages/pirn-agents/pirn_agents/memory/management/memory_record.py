...
pairs the record's :data:`~pirn_agents.memory.management.memory_kind.MemoryKind`
(episodic/semantic/procedural/profile) and text ``content`` with a
:class:`~pirn_agents.memory.management.memory_provenance.MemoryProvenance` and two
lifecycle signals used by decay and ranking: ``importance`` and ``last_accessed``.

It round-trips through the untyped
:class:`~pirn_agents.memory.stores.memory_store.MemoryStore` mapping interface via
