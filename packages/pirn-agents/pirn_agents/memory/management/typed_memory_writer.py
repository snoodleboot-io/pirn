...
:class:`~pirn_agents.memory.management.memory_record.MemoryRecord` with
:meth:`MemoryRecord.to_payload` and writes the resulting mapping under the
record's ``id`` via the untyped
:meth:`~pirn_agents.memory.stores.memory_store.MemoryStore.store` interface. Existing
...
class TypedMemoryWriter(MemoryWriterBase):
