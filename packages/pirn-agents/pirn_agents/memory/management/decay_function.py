...
Both the :class:`~pirn_agents.memory.management.decay_scorer.DecayScorer` knot and
the :class:`~pirn_agents.memory.management.low_value_eviction_policy.LowValueEvictionPolicy`
...
class DecayFunction:
    """Namespace for the shared importance x half-life recency decay math."""

    @staticmethod
    def score(importance: float, age_seconds: float, half_life_seconds: float) -> float:
