"""``_PlanExecutionResult`` — assemble the final ``AgentResponse`` from loop state."""
...
class _PlanExecutionResult(Knot):
    """Concatenate every step's result into the plan's final ``AgentResponse``."""

    def __init__(
        self,
        *,
        state: Knot | _PlanStepState,
        _config: KnotConfig,
        **kwargs: Any,
    ) -> None:
        super().__init__(state=state, _config=_config, **kwargs)

    async def process(self, state: _PlanStepState, **_: Any) -> AgentResponse:
