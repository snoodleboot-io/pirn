"""``YamlExtractorResultExtractor`` — final loop state to the public parsed mapping.

The loop's output is the accumulated ``YamlExtractorState``;
...
class YamlExtractorResultExtractor(Knot):
    ...
    async def process(self, state: YamlExtractorState, **_: Any) -> Mapping[str, Any]:
