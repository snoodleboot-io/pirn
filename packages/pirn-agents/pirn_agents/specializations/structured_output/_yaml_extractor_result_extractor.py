"""``_YamlExtractorResultExtractor`` — final loop state to the public parsed mapping.

The loop's output is the accumulated ``_YamlExtractorState``;
``YamlExtractorPipeline``'s contract is the bare parsed mapping (or a raised
...
class _YamlExtractorResultExtractor(Knot):
    ...
    async def process(self, state: _YamlExtractorState, **_: Any) -> Mapping[str, Any]:
        ...
        if state.result is None:
            raise ValueError(
                f"YamlExtractorPipeline: exhausted {state.attempts} attempt(s); "
