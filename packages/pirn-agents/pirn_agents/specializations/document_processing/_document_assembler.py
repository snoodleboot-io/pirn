"""``_DocumentAssembler`` — bytes-in, no-I/O half of the document loader split.

Pairs with :class:`_DocumentSource` (PIR-868): the source performs the guarded
...
:class:`_DocumentSource`'s construction and fetch.
...
Algorithm:
    ...
    3. Decode as UTF-8 (permissive: ``errors="replace"``, matching the
       decoding tolerance :class:`_DocumentSourceReader` already applied to
...
class _DocumentAssembler(Assembler):
    ...
    async def process(self, body: bytes, **_: Any) -> str:
        ...
        Args:
            body: The raw bytes produced by the upstream :class:`_DocumentSource`.
        ...
        if not isinstance(body, bytes):
            raise TypeError(f"_DocumentAssembler: body must be bytes, got {type(body).__name__}")
