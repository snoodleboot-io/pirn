...
straight onto this knot's output.
...
class InputGuardrailCheck(AgentPipeline):
        ...
        **_: Any,
        ...
        deny_compiled = [
            self._pattern_compiler.compile_safe_pattern(
                raw, index=i, owner="InputGuardrailCheck", field="deny_patterns"
            )
        ...
        for index, message in enumerate(message_tuple):
            if not isinstance(message, AgentMessage):
                raise TypeError(
                    f"InputGuardrailCheck: messages[{index}] must be an "
                    f"AgentMessage, got {type(message).__name__}"
                )
            for pattern in deny_compiled:
                if pattern.search(message.content):
                    raise ValueError(
                        f"InputGuardrailCheck: messages[{index}] matched deny "
