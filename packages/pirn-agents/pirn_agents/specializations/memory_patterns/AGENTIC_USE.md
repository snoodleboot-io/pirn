`pirn_agents.specializations.memory_patterns` provides pipelines for the four memory types used in agent systems — it does not store data itself; all persistence goes through a `MemoryStore` (vector store) and optionally a structured database pool.
...
```python
from pirn_agents.specializations.memory_patterns.semantic_memory_pipeline import SemanticMemoryPipeline
