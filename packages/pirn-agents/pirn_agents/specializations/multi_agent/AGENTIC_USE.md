`pirn_agents.specializations.multi_agent` provides patterns for coordinating multiple agents — it does not spawn processes or manage agent lifecycles; agents are knots that call an LLM, and coordination is wiring.
...
```
pirn_agents/specializations/multi_agent/
...
```python
from pirn_agents.specializations.multi_agent.orchestrator_agent import OrchestratorAgent
