...
``ConsensusAggregator``, which claimed core's :class:`~pirn.nodes.aggregator.Aggregator`
without being one — ``responses`` here arrives as a single already-assembled
...
(WS5b); ``ConsensusAggregator`` is kept importable as a one-cycle deprecation
shim (``consensus_aggregator.py``) forwarding to this class, per the ADR's
deprecation policy. The strategy this class *builds* for ``"majority_vote"``
...
class ConsensusPipeline(AgentPipeline):
    ...
    async def process(
        ...
        **_: Any,
        ...
        if strategy not in supported:
            raise ValueError(
                f"ConsensusPipeline: strategy must be one of {supported!r}, got {strategy!r}"
            )
        if not isinstance(responses, Mapping) or not responses:
            raise ValueError("ConsensusPipeline: responses must be a non-empty mapping")
        ...
        raise ValueError(f"ConsensusPipeline: no strategy matched {strategy!r}")
