    ...
    def digest(payload: Any) -> str:
        ...
        Args:
            payload: Any JSON-serialisable value. Mapping keys are sorted and
                separators are tight so the digest is independent of key order
                and incidental whitespace. Non-JSON leaves fall back to ``str``.
        ...
        # (PIR-795). Recorded cassettes are keyed by digests taken with the
        # `str` fallback, and STR_CONTENT renders every leaf `str` already
        # rendered as content byte-identically -- datetime, UUID, Decimal,
        # Path, Enum, and any type with its own __str__ -- so no cassette that
        # could be replayed moves.
        #
        # What it removes is the PIR-785 hazard: a leaf whose __str__ falls
        # through to the default __repr__ used to be digested by memory
        # address, so the cassette recorded from it could never replay -- the
        # address differs next run. That is now a TypeError at record time
        # instead of a silent miss at replay time. Nothing is orphaned,
        # because a key that never reproduced was never usable. RAISE would
        # go further and reject the content-rendering leaves too, which WOULD
        # be a cassette-format break.
