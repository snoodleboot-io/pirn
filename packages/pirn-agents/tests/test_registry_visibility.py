"""Registry-visibility acceptance test for pirn-agents (PIR-856).

``pirn_agents/__init__.py`` calls ``Registry.fill_registry`` at import time and
...
def _walk_knot_subclasses(package: object) -> tuple[set[str], list[str]]:
    ...
    module-scope alias (``OldName = NewName``, which ``scripts/check_conventions.py``
    rejects in package source) binds a second name to the *same* class object; sweet_tea's
...
def test_registry_holds_every_knot_subclass_when_extras_present() -> None:
    found, failed = _walk_knot_subclasses(pirn_agents)
    if failed:
        pytest.skip(
            "an optional dependency is missing for: "
            + ", ".join(sorted(failed))
            + " — install pirn-agents[all] to run this test"
        )

    registered = {
        f"{entry.class_def.__module__}.{entry.class_def.__qualname__}"
        for entry in Registry.typed_entries(Knot)
        if entry.class_def.__module__.startswith("pirn_agents.")
    }
