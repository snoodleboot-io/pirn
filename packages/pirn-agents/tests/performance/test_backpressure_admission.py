"""Unit tests for :class:`_BackpressureAdmission` (ADR agents-speaks-core, WS4b/PIR-866).
...
class TestConstruction:
    def test_rejects_non_config(self) -> None:
        with pytest.raises(TypeError, match="ConcurrencyConfig"):
            _BackpressureAdmission(object())  # type: ignore[arg-type]

    def test_is_an_admission_gate(self) -> None:
        assert isinstance(_BackpressureAdmission(ConcurrencyConfig()), AdmissionGate)
...
class TestBareGate:
    ...
    async def test_never_exceeds_max_concurrency(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=3))
        peak = 0
        live = 0
        lock = asyncio.Lock()
    ...
    def test_current_limit_has_no_group(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=4))
...
class TestGroupedGate:
    ...
    def test_tickets_carry_the_group_name(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=1), group="slow")
    ...
    def test_current_limit_reads_the_named_group(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=4), group="slow")
...
class TestBackpressureShedding:
    async def test_queue_full_raises_when_depth_exceeded(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=1, max_queue_depth=1))
        await pool.acquire()
...
class TestAcquireTimeout:
    async def test_timeout_raises_when_no_slot_frees(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=1, acquire_timeout=0.02))
        await pool.acquire()
...
class TestReleaseIdentity:
    ...
    async def test_releasing_one_of_two_equal_tickets_frees_only_that_one(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=2))
        await pool.acquire()
        await pool.acquire()
    ...
    def test_release_rejects_a_ticket_this_pool_never_issued(self) -> None:
        pool_a = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=1))
        pool_b = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=1))
        probe = _GateProbe(_config=KnotConfig(id="probe"))
    ...
    def test_release_with_no_ticket_and_nothing_held_raises(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig())
        with pytest.raises(AdmissionReleaseError):
            pool.release()
...
class TestAdmissionGateSurface:
    def test_has_capacity_and_set_limit(self) -> None:
        pool = _BackpressureAdmission(ConcurrencyConfig(max_concurrency=1))
        probe = _GateProbe(_config=KnotConfig(id="probe"))
        ticket = pool.try_admit(probe)
        assert ticket is not None
