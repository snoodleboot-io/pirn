"""Mirrored tests for the CI regression check (:class:`EvalRegressionCheck`, S6).
...
from pirn_agents.evaluation.eval_regression_check import EvalRegressionCheck
...
class EvalRegressionCheckTests(unittest.TestCase):
    def test_passing_run_clears_the_gate(self) -> None:
        ...
        result = EvalRegressionCheck(thresholds=_thresholds()).check(report)
    ...
    def test_baseline_regression_detected_even_above_threshold(self) -> None:
        ...
        result = EvalRegressionCheck(thresholds=thresholds).check(current, baseline=baseline)
        assert result.passed is False
    ...
    def test_verdict_json_has_breaches(self) -> None:
