"""End-to-end mirrored test for :meth:`RunEval.run` over a small fixture dataset.
...
class RunEvalTests(unittest.IsolatedAsyncioTestCase):
    async def test_end_to_end_scores_and_aggregates(self) -> None:
        ...
        report = await RunEval.run(dataset=_dataset(), target=target, metrics={"exact_match": em})
