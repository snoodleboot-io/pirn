"""Tests for :meth:`~pirn_agents.retrieval.reciprocal_rank_fusion.ReciprocalRankFusion.fuse`."""
...
class TestReciprocalRankFusion(unittest.TestCase):
    def test_rejects_non_positive_k(self) -> None:
        with self.assertRaises(ValueError):
            ReciprocalRankFusion.fuse([["a"]], k=0)
    ...
    def test_ties_break_by_first_appearance(self) -> None:
        ...
        fused = ReciprocalRankFusion.fuse([["x"], ["y"]])
        assert [identifier for identifier, _ in fused] == ["x", "y"]

    def test_larger_k_flattens_contribution(self) -> None:
        small = dict(ReciprocalRankFusion.fuse([["a", "b"]], k=1))
        large = dict(ReciprocalRankFusion.fuse([["a", "b"]], k=1000))
