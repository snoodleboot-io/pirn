...
- `ResolvedValueKnot`/`MessagesPassthrough`'s constant-seed use → `core/parameter.py`'s `Parameter` (16 call sites across 12 files migrated; both classes kept as docstring-deprecated shims — no runtime `DeprecationWarning` is possible for a Knot-shaped class, see their module docstrings for why: `Knot.run_scoped_copy`'s `copy.copy(self)` reconstructs via `cls.__new__(cls)` with no arguments on every run).
