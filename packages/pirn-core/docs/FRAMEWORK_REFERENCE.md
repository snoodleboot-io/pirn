...
**A schema can stand in for the signature.** `KnotFactory.from_schema(name, input_schema, process)` / `@KnotFactory.knot(input_schema=...)` declare the inputs of a knot that has no Python signature (an MCP-declared tool) with a JSON object schema; the generated class carries it (`Knot.declared_input_schema()` returns it) and `JsonSchemaTypeBuilder` turns each property into the `TypeAdapter` `validate_io` applies. The inverse, `Knot.input_json_schema()`, renders any knot's hinted inputs as the same kind of schema — the source for model-facing declarations.

**What a knot class accepts is a public classmethod API.** `Knot.reserved_kwargs()` (the framework's constructor kwargs, `_config`/`tapestry`), `Knot.declared_input_schema()` (a schema-declared class's input contract, else `None`), `Knot.input_annotations()` (name → the annotation each signature-declared input is validated with: `T` for `Knot | T`, `Any` when unannotated) and `Knot.input_json_schema()`. A wrapper that constructs a knot class it did not write — `ToolFactory`, `AgentTool` — reads these, never the protected machinery behind them (PIR-872).
...
- **Private-in-name-only is public (PIR-872).** Classes other modules import
  ...
  `NestedRunKnot` private-field writes they replaced),
