"""Registry-visibility acceptance test for pirn-core (PIR-856).

``pirn/__init__.py`` calls ``Registry.fill_registry`` at import time and now
logs — rather than silently swallowing — every module sweet_tea skips because
an optional dependency is missing. This test asserts the other half of that
contract: when every module in the package tree *does* import cleanly (all
extras present), the registry actually holds every ``Knot`` subclass shipped
in the package. If any module fails to import here, the environment is
