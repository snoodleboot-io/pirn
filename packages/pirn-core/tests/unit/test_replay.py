...
class _StandaloneTests(unittest.IsolatedAsyncioTestCase):
    async def test_replay_run_produces_same_result(self):
    ...
    async def test_replay_run_with_override_changes_output(self):
        history = SQLiteHistory(path=":memory:")
        t = build_tapestry(history=history)
    ...
    async def test_replay_run_raises_for_unknown_run_id(self):
        history = SQLiteHistory(path=":memory:")
        t = build_tapestry(history=history)
    ...
    async def test_replay_run_accepts_custom_run_id(self):
        history = SQLiteHistory(path=":memory:")
        t = build_tapestry(history=history)
...
# ----------------------------------------------------------------- compare_runs
    ...
    async def test_compare_runs_unchanged_knot_not_flagged(self):
        history = SQLiteHistory(path=":memory:")
        t = build_tapestry(history=history)
    ...
    def test_knot_diff_str_unchanged(self):
        d = KnotDiff("my_knot", "ok", "ok", "abc", "abc")
        assert str(d) == "= my_knot"
    
    
    def test_knot_diff_str_hash_changed(self):
        d = KnotDiff("my_knot", "ok", "ok", "abc", "xyz")
        assert "~" in str(d)
        assert "hash changed" in str(d)
    
    
    def test_knot_diff_str_outcome_changed(self):
