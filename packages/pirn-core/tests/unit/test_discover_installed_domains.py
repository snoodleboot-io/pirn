"""Tests for :meth:`pirn.domain_discovery.DomainDiscovery.discover_installed_domains` (SCD-19)."""
...
class TestDiscoverInstalledDomains(unittest.TestCase):
    def test_importable_from_public_module(self) -> None:
        ...
        self.assertTrue(callable(DomainDiscovery.discover_installed_domains))

    def test_returns_sorted_tuple_of_import_names(self) -> None:
        result = DomainDiscovery.discover_installed_domains()
    ...
    def test_is_idempotent(self) -> None:
        first = DomainDiscovery.discover_installed_domains()
        second = DomainDiscovery.discover_installed_domains()
        self.assertEqual(first, second)

    def test_cross_domain_knots_resolve_by_bare_name(self) -> None:
        DomainDiscovery.discover_installed_domains()
