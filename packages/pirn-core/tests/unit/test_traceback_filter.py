...
class _StandaloneTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_exception_manager_applies_filter(self) -> None:
    ...
    def test_exception_manager_no_filter_stores_verbatim(self) -> None:
...
class _EmitterPolicyTests(unittest.IsolatedAsyncioTestCase):
    """M-5: EmitterErrorPolicy behaviour when an emitter raises.

    These three tests previously sat on ``_BrokenEmitter`` — a plain helper class,
    not a TestCase — so pytest never collected them and they had never run
    (PIR-762). They also used the pytest ``caplog`` fixture, which a unittest-style
    class cannot receive at all; ``assertLogs``/``assertNoLogs`` are the
    ...
    async def test_emitter_policy_ignore_swallows_error(self) -> None:
        """IGNORE: no exception escapes and nothing is logged."""
        t = self._single_source_tapestry()
        with self.assertNoLogs("pirn.engine.engine", level=logging.WARNING):
            result = await t.run(
    ...
    async def test_emitter_policy_warn_logs_warning(self) -> None:
        ...
        assert result.succeeded
        assert any("BrokenEmitter" in m or "boom" in m for m in captured.output)

    async def test_emitter_policy_raise_propagates_exception(self) -> None:
        """RAISE: the emitter's exception propagates out of run()."""
        t = self._single_source_tapestry()
