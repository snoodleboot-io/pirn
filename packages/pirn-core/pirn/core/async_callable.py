...
class AsyncCallable:
    """Static helpers for reasoning about async-ness of arbitrary callables."""
    ...
    def is_async_callable(candidate: Callable[..., Any]) -> bool:
        ...
        Neither form sees through a callable *object* whose ``__call__`` carries
        the async, so that case is checked explicitly. Missing it is not cosmetic
        — a node that treats an async combine as sync emits a coroutine object as
        its output instead of the computed value, silently, since a coroutine is
        a perfectly good ``Any``.
