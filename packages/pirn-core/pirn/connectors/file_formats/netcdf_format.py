...
class NetcdfFormat(BatchFileFormat):
    ...
    def _records_to_structured_array(self, records: list[dict[str, Any]]) -> npt.NDArray[np.void]:
        ...
        field_dtypes = {field: np.dtype(spec) for field, spec in dtype_fields}
            ...
            for field in field_order:
                if field in record:
                    ...
                    structured[index][field] = (
                        value.encode("utf-8")
                        if isinstance(value, str) and field_dtypes[field].kind == "S"
                        else value
                    )
                else:
                    structured[index][field] = self._zero_for_dtype(field_dtypes[field])
        return structured
    ...
    @staticmethod
    def _zero_for_dtype(dtype: np.dtype[Any]) -> bytes | str | bool | int | float:
        kind = dtype.kind
        if kind in ("U", "S"):
