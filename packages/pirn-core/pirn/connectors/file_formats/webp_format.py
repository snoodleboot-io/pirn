...
single-image raster container that requires whole-payload decoding, so
this is a :class:`BatchFileFormat`.
...
rest — pirn's WebP path does not currently emit animated WebP.

Round-trip behaviour depends on the ``lossless`` flag:

* ``lossless=True``  — pixel data is preserved byte-for-byte.
* ``lossless=False`` (default) — codec is lossy; only ``width``,
  ``height`` and ``mode`` survive verbatim. Tests must not compare
  ``data`` byte-for-byte in lossy mode.
...
class WebpFormat(BatchFileFormat):
    ...
    async def _encode_full(self, records: Iterable[Mapping[str, Any]]) -> bytes:
        ...
        if not materialised:
            raise ValueError(
                "WebpFormat: cannot encode an empty record stream — "
                "WebP requires exactly one image per file."
            )
    ...
    def _validate_record(
        ...
        for field in ("width", "height", "mode", "data"):
            if field not in record:
                raise ValueError(
                    "WebpFormat: record missing required field "
