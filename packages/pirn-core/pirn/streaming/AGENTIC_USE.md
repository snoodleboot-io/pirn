...
A `StreamingSource` is an async generator that yields values over time. The `source.run_stream(tapestry)` driver binds each value to the parameter named `source.parameter_name` and calls `tapestry.run()` for that tick. The driver exits when the source is exhausted or when cancelled.
...
│                        StreamingSource.run_stream()          — driver: tick tapestry once per yielded value
...
└── trigger_adapter.py   StreamingSourceTrigger — adapt a StreamingSource into a Trigger for Trigger.run_forever
```
...
async def main():
    await source.run_stream(t, on_result=lambda v, r: print(r.outputs["score"]))
...
### Using `StreamingSource.run_stream` where `Trigger.run_forever` is correct

If each event should be an independent job with its own full `RunRequest` (different parameters per event, not just one changing value), use a `Trigger` with `Trigger.run_forever`. `StreamingSource.run_stream` is for when one value from the source is *the* input and everything else is constant.

### Not exhausting the source before closing

`StreamingSource.run_stream` calls `source.close()` on any exit, including cancellation. If you wrap the source in a context manager elsewhere, ensure `close()` is idempotent — it will be called once by `StreamingSource.run_stream` regardless.
...
| Task | How |
|------|-----|
| Tick tapestry on each stream value | `await source.run_stream(tapestry)` |
...
| Pass constants alongside stream values | `source.run_stream(tapestry, extra_parameters={"k": v})` |
| Observe per-tick results | `source.run_stream(tapestry, on_result=async_fn)` |
| Use a StreamingSource with Trigger.run_forever | `StreamingSourceTrigger(source=...)` |
