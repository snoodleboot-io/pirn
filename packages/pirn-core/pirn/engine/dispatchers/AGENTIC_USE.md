...
- **Dispatcher is tapestry-level, not knot-level.** All knots in a tapestry use the same dispatcher. To mix (e.g. most knots async, one CPU-bound), use `SubTapestry` with a different dispatcher on the inner tapestry. **Exception — agent-as-tool workloads:** a dispatcher on an *inner* tapestry is unsafe when the subtree runs agents-as-tools, because it crosses the `RunNesting` frame and `AgentToolPolicy` binds and silently defeats the depth/cycle/budget guards; set a `ThreadDispatcher` on the *outer* `Tapestry` instead. See [Concurrency and Dispatchers](../../../../../docs/guides/agentic-loops.md#concurrency-and-dispatchers).
