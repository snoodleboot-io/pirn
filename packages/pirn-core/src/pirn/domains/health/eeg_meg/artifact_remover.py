...
Algorithm:
    1. Receive a HealthSignalPayload, n_components int, and method string.
    ...
    5. Return a HealthSignalPayload with reconstructed data.
...
class ArtifactRemover(Knot):
    ...
    async def process(
        ...
        **_: Any,
        ...
        """Apply ICA-based artifact removal to the signal payload and return cleaned HealthSignalPayload.

        Args:
            signal: The HealthSignalPayload to clean.
        ...
        Returns:
            A HealthSignalPayload with ICA reconstruction applied.

        Raises:
            TypeError: If signal is not a HealthSignalPayload or n_components is not int.
            ValueError: If n_components is not positive or method is invalid.
        """
        if not isinstance(signal, HealthSignalPayload):
            raise TypeError("ArtifactRemover: signal must be a HealthSignalPayload")
