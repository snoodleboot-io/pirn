...
async def _record_divergence_and_combine(
    ...
    divergence_id = f"divergence:{digest[:16]}"
...
class ShadowDeploymentPipeline(SubTapestry):
    ...
    async def process(
        ...
        **_: Any,
        ...
        deploy_challenger = ShadowDeployer(
            model=challenger_node,
            registry=lineage,
            _config=KnotConfig(id="deploy-challenger"),
        )
        return _record_divergence_and_combine(
            champion_deployment_id=deploy_champion,
            challenger_deployment_id=deploy_challenger,
            champion=champion_node,
            challenger=challenger_node,
            lineage=lineage_node,
