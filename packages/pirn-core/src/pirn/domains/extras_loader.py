"""Per-domain optional-extras loader.

Each ``pirn.domains.<name>`` package instantiates :class:`ExtrasLoader` with
its required modules and calls :meth:`ExtrasLoader.require` at import time
...
class ExtrasLoader:
    ...
    Construct one per domain and call :meth:`require` from the domain's
    ``__init__.py``. Subclasses are not expected — the configuration is
    constructor-driven.
    """
    ...
    def require(self) -> None:
        ...
        pretty = ", ".join(missing)
        raise ImportError(
            f"pirn.domains.{self._extra_name} requires {pretty}. "
            f"Install with: pip install 'pirn[{self._extra_name}]'"
        )
