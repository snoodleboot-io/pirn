"""``DatabaseTableFreshnessCheck`` — fail if a table's max timestamp exceeds the SLA.
    ...
    async def process(
        ...
        **_: Any,
        ...
        if not isinstance(timestamp_column, str) or not timestamp_column:
            raise ValueError(
                "DatabaseTableFreshnessCheck: timestamp_column must be a non-empty string"
            )
        ...
        if not isinstance(max_age_seconds, int) or max_age_seconds < 1:
            raise ValueError(
                "DatabaseTableFreshnessCheck: max_age_seconds must be a positive integer"
            )
        rows = await pool.fetch_all(f"SELECT MAX({timestamp_column}) FROM {monitored_table}")
        max_ts_raw = rows[0][0]
        if max_ts_raw is None:
            raise RuntimeError(
                f"DatabaseTableFreshnessCheck: {monitored_table}.{timestamp_column} "
        ...
        if isinstance(max_ts_raw, str):
            ...
            raise TypeError(
                f"DatabaseTableFreshnessCheck: unexpected timestamp type {type(max_ts_raw)}"
            )
