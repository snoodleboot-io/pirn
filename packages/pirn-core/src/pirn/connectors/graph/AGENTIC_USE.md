`pirn.connectors.graph` provides `ConnectionConfig` and connection pool implementations for Neo4j, Memgraph, and OrientDB — it does not execute queries; pass the pool to `DatabaseQuerySource` / `DatabaseExecuteSink` with Cypher or OrientDB SQL query strings.
