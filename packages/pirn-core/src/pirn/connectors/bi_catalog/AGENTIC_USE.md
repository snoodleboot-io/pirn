`pirn.connectors.bi_catalog` provides API clients for data catalog and integration platforms (dbt, Fivetran, Airbyte, DataHub, Alation, OpenMetadata) — it does not run dbt models or trigger syncs inline; it exposes their REST APIs so pipeline knots can read metadata, trigger runs, and publish lineage.
...
**Using `AirbyteClient` as a real-time streaming source** — Airbyte syncs are batch operations. For continuous streaming, use native connector sources in `pirn.connectors.streaming`.
