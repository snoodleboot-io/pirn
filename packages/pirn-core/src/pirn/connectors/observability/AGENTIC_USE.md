`pirn.connectors.observability` provides clients for Datadog, Grafana, Prometheus, and OpenTelemetry — it does not replace `pirn.emitters`; these clients are for pushing metrics and events from within pipeline knots, not for tapestry-level lifecycle observability.
