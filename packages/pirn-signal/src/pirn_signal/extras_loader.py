"""Per-domain optional-extras loader for ``pirn_signal``.

The ``pirn_signal`` package instantiates :class:`ExtrasLoader` with its
...
class ExtrasLoader:
    ...
    def require(self) -> None:
        ...
        pretty = ", ".join(missing)
        raise ImportError(
            f"pirn_signal[{self._extra_name}] requires {pretty}. "
            f"Install with: pip install 'pirn-signal[{self._extra_name}]'"
        )
