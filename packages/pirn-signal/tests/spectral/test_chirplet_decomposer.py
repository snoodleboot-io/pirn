...
from pirn_signal.spectral.chirplet_decomposer import ChirpletDecomposer
...
class TestConstruction(unittest.IsolatedAsyncioTestCase):
    async def test_rejects_non_positive_chirplet_count(self) -> None:
        with Tapestry():
            k = ChirpletDecomposer.__new__(ChirpletDecomposer)
            object.__setattr__(k, "_config", KnotConfig(id="c"))
        signal = _make_signal_payload()
        with self.assertRaises((TypeError, ValueError)):
            await k.process(signal=signal, chirplet_count=0)
