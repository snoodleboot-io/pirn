"""Every optional import in pirn_signal names this package and one of its real extras.

pirn_signal imports its optional backends through
:meth:`pirn.core.optional_dependency.OptionalDependency.require`. The install hint
that call raises is only useful if it names ``pirn-signal`` and an extra that
...
class TestOptionalDependencyCallSites:
    ...
    def _require_calls() -> list[tuple[str, ast.Call]]:
        ...
        for package_dir in pirn_signal.__path__:
    ...
    def test_every_call_names_this_package_and_a_declared_extra(self) -> None:
        ...
        for location, call in self._require_calls():
            ...
            if not (isinstance(package, ast.Constant) and package.value == "pirn-signal"):
                wrong.append(f"{location}: package is not 'pirn-signal'")
    ...
    def test_a_missing_module_names_the_package_extra(self) -> None:
        ...
        # Act / Assert
        with pytest.raises(ImportError, match=rf'pip install "pirn-signal\[{extra}\]"'):
            OptionalDependency.require(
                "pirn_signal_absent_backend", extra=extra, package="pirn-signal"
            )
