"""Registry-visibility acceptance test for pirn-signal (PIR-856).

``pirn_signal/__init__.py`` calls ``Registry.fill_registry`` at import time and
...
def test_registry_holds_every_knot_subclass_when_extras_present() -> None:
    found, failed = _walk_knot_subclasses(pirn_signal)
    if failed:
        pytest.skip(
            "an optional dependency is missing for: "
            + ", ".join(sorted(failed))
            + " — install pirn-signal[signal] to run this test"
        )

    registered = {
        f"{entry.class_def.__module__}.{entry.class_def.__name__}"
        for entry in Registry.typed_entries(Knot)
        if entry.class_def.__module__.startswith("pirn_signal.")
    }
