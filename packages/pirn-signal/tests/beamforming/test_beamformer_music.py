...
from pirn_signal.beamforming.beamformer_music import BeamformerMUSIC
...
class TestBeamformerMUSIC(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_positive_num_elements(self) -> None:
        ...
        with pytest.raises(ValueError, match="num_elements"):
            await knot.process(
                _SIGNAL, num_elements=0, element_spacing_m=0.05, num_sources=2, n_grid=181
            )
    ...
    async def test_rejects_non_positive_num_sources(self) -> None:
        ...
        with pytest.raises(ValueError, match="num_sources"):
            await knot.process(
                _SIGNAL, num_elements=8, element_spacing_m=0.05, num_sources=0, n_grid=181
            )
    ...
    async def test_rejects_zero_step(self) -> None:
        ...
        with pytest.raises(ValueError, match="n_grid"):
            await knot.process(
                _SIGNAL, num_elements=8, element_spacing_m=0.05, num_sources=2, n_grid=0
            )

    async def test_emits_spectrum_frame(self) -> None:
        knot = self._make()
        out = await knot.process(
            _SIGNAL, num_elements=8, element_spacing_m=0.05, num_sources=2, n_grid=181
        )
