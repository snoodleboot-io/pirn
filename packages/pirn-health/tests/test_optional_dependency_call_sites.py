"""Every optional import in pirn_health names this package and one of its real extras.

pirn_health imports its optional backends through
:meth:`pirn.core.optional_dependency.OptionalDependency.require`. The install hint
that call raises is only useful if it names ``pirn-health`` and an extra that
...
class TestOptionalDependencyCallSites:
    ...
    def _require_calls() -> list[tuple[str, ast.Call]]:
        ...
        for package_dir in pirn_health.__path__:
    ...
    def test_every_call_names_this_package_and_a_declared_extra(self) -> None:
        ...
        for location, call in self._require_calls():
            ...
            if not (isinstance(package, ast.Constant) and package.value == "pirn-health"):
                wrong.append(f"{location}: package is not 'pirn-health'")
    ...
    def test_a_missing_module_names_the_package_extra(self) -> None:
        ...
        # Act / Assert
        with pytest.raises(ImportError, match=rf'pip install "pirn-health\[{extra}\]"'):
            OptionalDependency.require(
                "pirn_health_absent_backend", extra=extra, package="pirn-health"
            )
