...
_KNOT = EEGMontageApplier(
    signal=_SIGNAL, montage_name="standard_1020", reference="average", _config=_CFG
)
...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_drop_channels_reduces_count(self) -> None:
        out_full = await _KNOT.process(
            signal=_SIGNAL, montage_name="standard_1020", reference="average"
        )
        out_dropped = await _KNOT.process(
            signal=_SIGNAL,
