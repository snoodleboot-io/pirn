...
def _make_knot() -> TaskFMRIModeler:
    with Tapestry():
        ...
        return TaskFMRIModeler(
            bold_data=bd, events=ev, tr_sec=2.0, hrf_model="spm", high_pass_hz=0.01, _config=_CFG
        )
...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    async def test_rejects_invalid_hrf_model(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "hrf_model"):
            await knot.process(
                bold_data=_BOLD, events=_EVENTS, tr_sec=2.0, hrf_model="gamma", high_pass_hz=0.01
            )
    ...
    async def test_rejects_non_positive_tr(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "tr_sec"):
            await knot.process(
                bold_data=_BOLD, events=_EVENTS, tr_sec=0.0, hrf_model="spm", high_pass_hz=0.01
            )
    ...
    async def test_rejects_non_positive_high_pass(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "high_pass_hz"):
            await knot.process(
                bold_data=_BOLD, events=_EVENTS, tr_sec=2.0, hrf_model="spm", high_pass_hz=0.0
            )

    async def test_returns_dict(self) -> None:
        knot = _make_knot()
        out = await knot.process(
            bold_data=_BOLD, events=_EVENTS, tr_sec=2.0, hrf_model="spm", high_pass_hz=0.01
        )
