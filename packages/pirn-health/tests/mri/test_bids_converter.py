...
def _make_knot() -> BIDSConverter:
    with Tapestry():
        ...
        return BIDSConverter(
            input_data=src, output_dir="/bids", modality="T1w", subject_id="001", _config=_CFG
        )
...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_invalid_modality(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "modality"):
            await knot.process(
                input_data=_DATA, output_dir="/bids", modality="CT", subject_id="001"
            )
    ...
    async def test_returns_dict(self) -> None:
        ...
        out = await knot.process(
            input_data=_DATA, output_dir="/bids", modality="T1w", subject_id="001"
        )
