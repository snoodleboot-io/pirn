...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    async def test_rejects_invalid_template(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "template"):
            await knot.process(
                image=_IMAGE, template="MNI305", registration_type="linear", degrees_of_freedom=12
            )
    ...
    async def test_rejects_invalid_registration_type(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "registration_type"):
            await knot.process(
                image=_IMAGE, template="MNI152", registration_type="affine", degrees_of_freedom=12
            )
    ...
    async def test_rejects_invalid_dof(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "degrees_of_freedom"):
            await knot.process(
                image=_IMAGE, template="MNI152", registration_type="linear", degrees_of_freedom=3
            )
    ...
    async def test_returns_dict(self) -> None:
        ...
        with patch("asyncio.create_subprocess_exec", AsyncMock(return_value=mock_proc)):
            out = await knot.process(
                image=_IMAGE, template="MNI152", registration_type="linear", degrees_of_freedom=12
            )
