...
def _make_knot() -> MRIQualityController:
    with Tapestry():
        ...
        return MRIQualityController(
            mri_data=src, snr_threshold=10.0, motion_threshold_mm=0.5, modality="T1w", _config=_CFG
        )
...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    async def test_rejects_invalid_modality(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "modality"):
            await knot.process(
                mri_data=_MRI, snr_threshold=10.0, motion_threshold_mm=0.5, modality="CT"
            )
    ...
    async def test_rejects_non_positive_snr_threshold(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "snr_threshold"):
            await knot.process(
                mri_data=_MRI, snr_threshold=0.0, motion_threshold_mm=0.5, modality="T1w"
            )
    ...
    async def test_rejects_non_positive_motion_threshold(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "motion_threshold_mm"):
            await knot.process(
                mri_data=_MRI, snr_threshold=10.0, motion_threshold_mm=0.0, modality="T1w"
            )

    async def test_returns_dict(self) -> None:
        knot = _make_knot()
        out = await knot.process(
            mri_data=_MRI, snr_threshold=10.0, motion_threshold_mm=0.5, modality="T1w"
        )
