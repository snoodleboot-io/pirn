...
from pirn_health.genomics.snpeff_annotator import SnpEffAnnotator
...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_string(self) -> None:
        ...
        with self.assertRaisesRegex(TypeError, "vcf_path"):
            await knot.process(vcf_path=42, genome_db="GRCh38", output_vcf_path="out")
    ...
    async def test_returns_annotated_path(self) -> None:
        ...
        with patch("asyncio.create_subprocess_exec", AsyncMock(return_value=mock_proc)):
            out = await knot.process(
                vcf_path="in.vcf", genome_db="GRCh38", output_vcf_path="out.vcf"
            )
        assert out == "out.vcf"
