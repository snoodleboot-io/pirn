...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_zero_min_length(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "min_length"):
            await knot.process(
                fastq=_FASTQ, adapter_sequence="AGATCGGAA", min_length=0, quality_cutoff=20
            )
    ...
    async def test_rejects_quality_cutoff_too_high(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "quality_cutoff"):
            await knot.process(
                fastq=_FASTQ, adapter_sequence="AGATCGGAA", min_length=20, quality_cutoff=50
            )
    ...
    async def test_rejects_negative_quality_cutoff(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "quality_cutoff"):
            await knot.process(
                fastq=_FASTQ, adapter_sequence="AGATCGGAA", min_length=20, quality_cutoff=-1
            )

    async def test_returns_dict(self) -> None:
        knot = _make_knot()
        out = await knot.process(
            fastq=_FASTQ, adapter_sequence="AGATCGGAA", min_length=20, quality_cutoff=20
        )
