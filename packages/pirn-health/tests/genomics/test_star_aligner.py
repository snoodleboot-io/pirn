...
from pirn_health.genomics.star_aligner import STARAligner
...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_non_string(self) -> None:
        ...
        with self.assertRaisesRegex(TypeError, "fastq_path"):
            await knot.process(fastq_path=42, genome_dir="g", output_bam_path="out")
    ...
    async def test_returns_bam_path(self) -> None:
        ...
        with patch("asyncio.create_subprocess_exec", AsyncMock(return_value=mock_proc)):
            out = await knot.process(
                fastq_path="in.fastq", genome_dir="g", output_bam_path="out.bam"
            )
        assert out == "out.bam"
