...
class PathologyStainNormalizer(Knot):
    ...
    async def process(
        ...
        **_: Any,
        ...
        return await asyncio.to_thread(
            self._normalize_stain, image_tile, method, reference_stain_matrix
        )
    ...
    def _macenko_normalize(
        ...
        od = PathologyStainNormalizer._to_od(pixels)
    ...
    def _normalize_stain(
        ...
        if method in ("macenko", "vahadane"):
            norm_pixels, mat = PathologyStainNormalizer._macenko_normalize(
                pixels, reference_stain_matrix
            )
        else:
            norm_pixels, mat = PathologyStainNormalizer._reinhard_normalize(
                pixels, reference_stain_matrix
            )
        return {
