...
│   └── fhir_patient_assembler.py             — list[dict] + salt → tuple[ClinicalRecord, ...] (hashes patient_id/encounter_id)
├── disassemblers/
│   ├── __init__.py
...
- **Sub-domain knots are importable stubs.** Heavy vendor SDKs (`pydicom`, `mne`, `nibabel`, `pyfaidx`, `pysam`, `fhir.resources`) are not imported at package load time. The `pirn_health.*` knot classes are importable without the extra installed, but calling `process()` will fail at runtime if the underlying SDK is missing.
