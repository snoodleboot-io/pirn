    ...
    async def process(
        ...
        **_: Any,
        ...
        if not isinstance(records, (list, tuple)):
            raise TypeError("ClinicalDataQualityCheck: records must be list/tuple")
        for record in records:
            if not isinstance(record, ClinicalRecord):
                raise TypeError("ClinicalDataQualityCheck: every record must be a ClinicalRecord")
        if not isinstance(min_completeness, (int, float)):
            raise TypeError("ClinicalDataQualityCheck: min_completeness must be numeric")
        if not 0.0 <= float(min_completeness) <= 1.0:
            raise ValueError("ClinicalDataQualityCheck: min_completeness must be in [0, 1]")
