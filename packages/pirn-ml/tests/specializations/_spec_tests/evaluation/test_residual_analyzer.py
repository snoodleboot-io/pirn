...
from pirn_ml.specializations.evaluation.residual_analyzer import (
    ResidualAnalyzer,
)
...
def _fixtures():
    ...
    model = ModelManifest(model_id="m1", algorithm="ridge", feature_names=("a",), target_name="y")
    return model, split
...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_rejects_float_n_bins(self) -> None:
        ...
        model, split = _fixtures()
        with self.assertRaises((TypeError, ValueError)):
            await k.process(model=model, split=split, n_bins=5.5)
