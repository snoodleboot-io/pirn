...
def _fixtures():
    ...
    model = ModelManifest(model_id="m1", algorithm="cnn", feature_names=("a",), target_name="y")
    return model, split
...
class TestProcess(unittest.IsolatedAsyncioTestCase):
    async def test_rejects_invalid_attack(self) -> None:
        with Tapestry():
            k = AdversarialRobustnessEvaluator.__new__(AdversarialRobustnessEvaluator)
            object.__setattr__(k, "_config", KnotConfig(id="x"))
        model, split = _fixtures()
        with self.assertRaises((TypeError, ValueError)):
            await k.process(model=model, split=split, attack="invalid", epsilon=0.1)
...
class TestHappyPath(unittest.IsolatedAsyncioTestCase):
    async def test_emits_accuracy_metrics(self) -> None:
        with Tapestry() as t:
        ...
        out = result.outputs["eval"]
        assert "clean_accuracy" in out
        assert "adversarial_accuracy" in out
        assert out["attack"] == "pgd"
