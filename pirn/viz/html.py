...
"""
    ...
    _document: ClassVar[str] = """<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>{title}</title>
...
"""
    ...
    @classmethod
    def for_tapestry(cls, tapestry: Tapestry, title: str | None = None) -> str:
        ...
        nodes = [
            {
                "id": k.knot_id,
                "class": cls._short(type(k).__qualname__),
                "outcome": "pending",
                "duration_ms": 0,
                "output_hash": "",
                "config_hash": "",
        ...
        layers = cls._layer_nodes(nodes, edges)
        coords = cls._assign_coordinates(layers)
        svg = cls._render_svg(nodes, edges, coords)

        summary = (
            f'<div class="summary">'
            f'<span class="label">pipeline</span> {_html_module.escape(title)}'
            f' &nbsp;·&nbsp; <span class="label">knots</span> {len(knots)}'
            f"</div>"
        )
        return cls._document.format(
    ...
    @classmethod
    def for_run(cls, result: RunResult, title: str | None = None) -> str:
        ...
        nodes = [
            {
                "id": rec.knot_id,
                "class": cls._short(rec.knot_class),
                "outcome": rec.outcome,
        ...
        # Render.
        summary_html = cls._render_summary(result)
        svg = cls._render_svg(nodes, edges, coords)
        return cls._document.format(
            title=_html_module.escape(title),
    ...
    def _render_summary(result: RunResult) -> str:
        ...
        return (
            ...
            f'<span class="value">{_html_module.escape(result.run_id)}</span></div>'
            ...
            f'<span class="value">{_html_module.escape(result.dispatcher)}</span></div>'
            f"</div>"
        )
    ...
    def _render_svg(
        ...
        for node in nodes:
            ...
            parts.append(
                ...
                f'data-output-hash="{_html_module.escape(node["output_hash"])}" '
                f'data-config-hash="{_html_module.escape(node["config_hash"])}" '
                f'data-error="{_html_module.escape(node["error_record_id"])}" '
                f'data-skip-reason="{_html_module.escape(node["skip_reason"])}">'
            )
