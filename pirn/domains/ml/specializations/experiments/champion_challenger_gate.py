    ...
    async def process(
        ...
        **_: Any,
        ...
        if not isinstance(primary_metric, str) or not primary_metric:
            raise ValueError("ChampionChallengerGate: primary_metric must be a non-empty string")
        if not isinstance(min_improvement, (int, float)):
            raise TypeError("ChampionChallengerGate: min_improvement must be a number")
        ...
        if not isinstance(champion_report, EvalReportPayload):
            raise TypeError(
                "ChampionChallengerGate: champion evaluator did not return an EvalReportPayload"
            )
        if not isinstance(challenger_report, EvalReportPayload):
            raise TypeError(
                "ChampionChallengerGate: challenger evaluator did not return an EvalReportPayload"
            )
