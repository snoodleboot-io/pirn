...
class EarlyStoppingTrainer(SubTapestry):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not isinstance(split, Knot):
            raise TypeError("EarlyStoppingTrainer: split must be a Knot")
        if not isinstance(algorithm, str) or not algorithm:
            raise ValueError(
                "EarlyStoppingTrainer: algorithm must be a non-empty string"
            )
        if not isinstance(monitor_metric, str) or not monitor_metric:
            raise ValueError(
                "EarlyStoppingTrainer: monitor_metric must be a non-empty "
                "string"
            )
        ...
        if hyperparameters is not None and not isinstance(
            ...
            raise TypeError(
                "EarlyStoppingTrainer: hyperparameters must be a Mapping"
            )
        ...
        self._metrics = tuple(metrics) if metrics else (monitor_metric,)
    ...
    async def process(
        ...
        """
        hp = {**self._hyperparameters, "max_epochs": self._max_epochs}
        ...
        return {
            "model": trained_model,
            "eval_report": report,
            "stopped_epoch": self._max_epochs,
