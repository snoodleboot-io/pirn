...
class CdaXmlFormat(BatchFileFormat):
    ...
    _phi_keywords: ClassVar[frozenset[str]] = frozenset({"name", "birthTime", "addr", "telecom"})
    ...
    @classmethod
    def _find_extension_attr(cls, root: Any, tag: str, attr: str) -> str:
        el = root.find(f"{{{CdaXmlFormat._cda_ns}}}{tag}")
        if el is None:
            return ""
    ...
    def _extract_body(cls, root: Any) -> dict[str, Any]:
        ...
        patient = root.find(f".//{{{CdaXmlFormat._cda_ns}}}patientRole")
