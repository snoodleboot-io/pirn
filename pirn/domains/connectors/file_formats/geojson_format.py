...
class GeoJsonFormat(StreamingFileFormat):
    ...
    def __init__(self, encoding: str = "utf-8") -> None:
        if not isinstance(encoding, str):
            raise TypeError(
                "GeoJsonFormat: encoding must be str, "
    ...
    async def read(
        ...
        if not payload.strip():
            ...
            if not isinstance(parsed, dict):
                raise ValueError(
                    "GeoJsonFormat: expected JSON object at root, got "
                    f"{type(parsed).__name__}"
                )
            type_value = parsed.get("type")
            if type_value != "FeatureCollection":
                raise ValueError(
                    "GeoJsonFormat: expected type='FeatureCollection', "
                    f"got {type_value!r}"
                )
            raw_features = parsed.get("features", [])
            if not isinstance(raw_features, list):
                raise ValueError(
                    "GeoJsonFormat: 'features' must be a list, got "
                    f"{type(raw_features).__name__}"
                )
            features = [
                self._feature_to_record(feature) for feature in raw_features
            ]
    ...
    def _feature_to_record(feature: Any) -> Mapping[str, Any]:
        if not isinstance(feature, dict):
            raise ValueError(
                "GeoJsonFormat: each feature must be a JSON object, got "
                f"{type(feature).__name__}"
            )
        if feature.get("type") != "Feature":
            raise ValueError(
                "GeoJsonFormat: feature missing type='Feature', got "
        ...
        if not isinstance(properties, dict):
            raise ValueError(
                "GeoJsonFormat: properties must be a JSON object, got "
        ...
        return {
            ...
            "feature_id": (
                None if feature_id is None else str(feature_id)
            ),
        }
    ...
    def _record_to_feature(record: Mapping[str, Any]) -> Mapping[str, Any]:
        if "geometry" not in record:
            raise ValueError(
                "GeoJsonFormat: record missing required 'geometry' field"
            )
        ...
        if not isinstance(properties, Mapping):
            raise TypeError(
                "GeoJsonFormat: properties must be a Mapping, got "
                f"{type(properties).__name__}"
            )
