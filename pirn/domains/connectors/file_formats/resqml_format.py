"""``ResqmlFormat`` — RESQML XML reservoir-description batch encoder/decoder.

RESQML (Reservoir Characterization and Simulation Markup Language) is an
XML-based Energistics standard for exchanging reservoir geometry, property,
and simulation data. HDF5 sidecar files (for array data) are out of scope
for this connector; only the XML envelope is processed.

One record is emitted per top-level child element as a flat
``dict[str, str]`` of ``{tag: text}`` pairs.

Writing reconstructs a minimal RESQML XML document.
