...
from check_no_import_forwarding import CheckNoImportForwarding
...
def test_flags_unused_re_export(tmp_path: Path) -> None:
    ...
    violations = CheckNoImportForwarding.check_file(init)
...
def test_allows_a_used_import(tmp_path: Path) -> None:
    ...
    assert CheckNoImportForwarding.check_file(init) == []
...
def test_re_export_in_all_is_still_forwarding(tmp_path: Path) -> None:
    ...
    init.write_text('from acme.widget import Widget\n\n__all__ = ["Widget"]\n')
    assert len(CheckNoImportForwarding.check_file(init)) == 1
...
def test_ignores_non_init_files(tmp_path: Path) -> None:
    ...
    assert CheckNoImportForwarding.check_file(mod) == []
...
def _run(monkeypatch: pytest.MonkeyPatch, *args: str) -> int:
    ...
    return CheckNoImportForwarding.main()
...
def test_resolve_paths_expands_a_directory(tmp_path: Path) -> None:
    ...
    files, errors = CheckNoImportForwarding.resolve_paths([str(tmp_path)])
    assert errors == []
...
def test_resolve_paths_skips_vendored_trees(tmp_path: Path) -> None:
    ...
    files, _ = CheckNoImportForwarding.resolve_paths([str(tmp_path)])
...
def test_non_python_file_is_an_error(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_clean_tree_exits_zero(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    root = _pkg(tmp_path, "acme")
    (root / "__init__.py").write_text('"""Docstring."""\n\n__all__: list[str] = []\n')
    assert _run(monkeypatch, str(tmp_path)) == 0


def test_file_arguments_still_work(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_duplicate_paths_are_scanned_once(tmp_path: Path) -> None:
    root = _pkg(tmp_path, "acme")
    init = root / "__init__.py"
    files, _ = CheckNoImportForwarding.resolve_paths([str(tmp_path), str(init)])
...
def test_allowlist_is_empty() -> None:
    ...
    assert CheckNoImportForwarding._allowlist == frozenset()


def test_an_unparsable_init_is_reported(tmp_path: Path) -> None:
    # The branch that used to carry `# pragma: no cover - defensive` (PIR-873): a gate
    # that cannot read a file must say so rather than silently pass it.
    root = _pkg(tmp_path, "acme")
    init = root / "__init__.py"
    init.write_text("from acme.widget import (\n")
    findings = CheckNoImportForwarding.check_file(init)
