...
A **Parameter** is the canonical entry point for data that varies between runs. It is also a knot (no parents, no process logic to implement) and its value is bound from `RunRequest(parameters={"name": value})` at run time. The engine resolves the full graph in dependency order, starting each knot as soon as its parents are done, caches intermediate values by content hash, and stores a `KnotLineage` record per knot per run. Each result is one of `Ok(value)`, `Err(record)`, or `Skipped(reason)` — never a bare exception. By default, if a parent produces `Err` or `Skipped`, downstream knots are skipped rather than crashed.
