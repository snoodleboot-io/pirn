"""ATDD acceptance test: Pandera validation embedded in a Tier-2 Pandas
pipeline.

Mirrors :mod:`tests.integration.domains.data.test_pandera_polars_acceptance`
but uses the original Pandas-flavoured pandera API and the Pandas Tier-2
transforms (``PandasFilter``, ``PandasAggregate`` with
:class:`AggregateSpec`).

1. Source emits a Tier-1 :class:`DataBatch`.
2. Bridge to Tier-2 :class:`PandasDataBatch`.
3. ``PanderaPandasValidator`` validates against a rich Pandera schema
