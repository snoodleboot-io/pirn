...
def test_typing_union_also_detected():
    assert "step" in TypingUnionRouter._coercible_params


def test_nullable_union_coercible():
...
def test_scalar_becomes_parameter_in_tapestry():
    with Tapestry() as t:
        router = StepRouter(
            step="lookup policy",
...
def test_auto_parameter_registered_in_tapestry():
    with Tapestry() as t:
...
def test_knot_parent_not_re_wrapped():
    with Tapestry() as t:
...
def test_coerced_scalar_value_flows_at_runtime():
    async def run() -> str:
        with Tapestry() as t:
