...
def test_valid_tapestry_passes():
    result = validate_tapestry(build_valid())
    assert result.ok
    assert not result.errors


def test_empty_tapestry_warns():
...
def test_no_false_positives_on_linear_chain():
    result = validate_tapestry(build_valid())
    assert not result.issues or all(i.severity == "warning" for i in result.issues)


def test_validation_issue_str_with_knot_id():
...
def test_validation_issue_str_without_knot_id():
