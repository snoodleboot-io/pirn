...
@pytest.mark.asyncio
async def test_single_aggregation(duckdb_orders) -> None:
    with Tapestry() as t:
        src = IbisSource(
...
@pytest.mark.asyncio
async def test_multiple_aggregations(duckdb_orders) -> None:
    with Tapestry() as t:
        src = IbisSource(
            connection=duckdb_orders, table="orders",
            backend_name="duckdb", _config=KnotConfig(id="src"),
...
@pytest.mark.asyncio
async def test_composite_group_by(duckdb_orders) -> None:
    with Tapestry() as t:
        src = IbisSource(
            connection=duckdb_orders, table="orders",
            backend_name="duckdb", _config=KnotConfig(id="src"),
...
def test_construct_rejects_string_by(duckdb_orders) -> None:
    with Tapestry():
        src = IbisSource(
            connection=duckdb_orders, table="orders",
...
def test_construct_rejects_empty_by(duckdb_orders) -> None:
    with Tapestry():
        src = IbisSource(
            connection=duckdb_orders, table="orders",
            _config=KnotConfig(id="src"),
