...
def test_empty_tapestry():
    t = Tapestry()
    assert t.all_knots() == []
    assert t.terminals() == []


def test_with_block_sets_contextvar():
...
def test_with_block_restores_outer_context():
...
def test_registration_via_with():
    with Tapestry() as t:
        p = Parameter("x", int)
...
def test_registration_via_explicit_kwarg():
    t = Tapestry()
    Parameter("x", int, tapestry=t)
    assert t.get("param:x") is not None


def test_terminals_simple_chain():
    with Tapestry() as t:
        p = Parameter("x", int)
        d = _f(x=p, _config=KnotConfig(id="d"))
...
def test_register_different_instance_same_id_raises():
    t = Tapestry()
...
async def test_run_with_no_terminals_raises():
    t = Tapestry()
...
def test_add_and_remove_emitter_by_identity():
...
def test_remove_emitter_raises_when_not_registered():
    from pirn.emitters.log import LogEmitter
...
def test_remove_emitter_uses_identity_not_equality():
...
def test_emitters_property_returns_copy():
    from pirn.emitters.log import LogEmitter
