...
- **Groups:** `KnotConfig(concurrency_group="openai")` puts a knot in a group; `ConcurrencyLimits(groups={"openai": 4})` lets at most four of them run at once, alongside any global cap. A group the limits do not list is bounded only by `max_in_flight`. `concurrency_group` is excluded from `model_dump`, so `knot_config_hash` is unchanged and existing recordings still replay.
- **Fairness:** `ReadyQueue` keeps one FIFO per group. A full group never blocks knots of other groups (no head-of-line blocking), and within a group knots start in readiness order.
